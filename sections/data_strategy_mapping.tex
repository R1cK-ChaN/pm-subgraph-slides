...
\begin{frame}{Entity Mapping: Price Parity Strategy}
    ...
    \begin{tikzpicture}[node distance=1.2cm, scale=0.8, transform shape,
        entity/.style={rectangle, draw, fill=nus-blue!20, text width=2.5cm, align=center, rounded corners, minimum height=0.8cm, font=\footnotesize},
        use/.style={rectangle, draw, fill=nus-orange!25, text width=2.8cm, align=center, rounded corners, minimum height=0.8cm, font=\footnotesize},
        ...
        \draw[arrow, dashed, color=nus-blue] (trade.east) -- ++(0.8,0) |- (identify.west);
        \draw[arrow, dashed, color=nus-blue] (split.east) -- ++(0.8,0) |- (validate.west);
    \end{tikzpicture}
    \end{center}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Feature Engineering}
                \footnotesize
                \textbf{Trade:} Price momentum, volume, volatility

                \textbf{Market:} Total volume, trader count, time to resolution

                \textbf{User:} Whale participation, concentration
            \end{block}
        \end{column}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Label Generation}
                \footnotesize
                \textbf{From winningOutcome:} Binary, multi-class, or regression labels
                ...
                \begin{tabular}{ccc}
                    ...
                    \midrule
                    $[0.6, 10k]$ & Yes & Trade+Market \\
                    $[0.3, 50k]$ & No & Trade+Market \\
                    \bottomrule
                \end{tabular}
            \end{block}
        \end{column}
...
\begin{frame}{Cross-Strategy Data Reuse}
    ...
    \begin{tikzpicture}[scale=0.85, transform shape]
        ...
        % Arrows
        \draw[thick, <->, >=stealth, color=nus-blue] (hub) -- (micro);
        \draw[thick, <->, >=stealth, color=nus-blue] (hub) -- (parity);
        \draw[thick, <->, >=stealth, color=nus-blue] (hub) -- (ml);
        \draw[thick, <->, >=stealth, color=nus-blue] (hub) -- (semantic);
    \end{tikzpicture}
    \end{center}
